...
export class DatabaseStatusBarItem implements Disposable {
    ...
    update() {
        let doc = window.activeTextEditor && (window.activeTextEditor.document.languageId === 'sql' || window.activeTextEditor.document.languageId === 'sqlite' || window.activeTextEditor.document.isUntitled)? window.activeTextEditor.document : undefined;
